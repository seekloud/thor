...
object CanvasUtils {
  ...
  def rotateImage(typ: String, drawFrame: MiddleFrame, ctx: MiddleContext, preCanvas: List[MiddleCanvas] = Nil, src:String, position: Point, offset:Point,
      ...
      preCanvas match {
        ...
        case _ =>
          if(System.currentTimeMillis() - preTime < 2000)
            ...
            typ match {
              case "adventurer" => drawAdventurerByPre(ctx, preCanvas, level, Point(-width/2 + offset.x, -drawHeight/2 + offset.y), Point(width, drawHeight))
              case "weapon" => drawWeaponByPre(ctx, preCanvas, level, Point(-width/2 + offset.x, -drawHeight/2 + offset.y), Point(width, drawHeight))
              case _ => ()
            }
          }
